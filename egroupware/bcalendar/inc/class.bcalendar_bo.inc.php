...
class bcalendar_bo
{
	...
	{
		$params_in = $params;
                $params['cfs'] = array('kolor','image_vip', 'image_wazne', 'image_pacjent_z_bolem', 'image_brak_diagnozy', 
                                       'image_potwierdzic', 'image_potwierdzone', 'image_ostroznie_roszczeniowy', 
                                       'image_ostroznie_niebezpieczny',
                                       'image_potwierdzic_prace'); //pola niestandardowe do pobrania z bazy danych
	...
	function add_adjusted_event(&$events,$event,$date_ymd)
	{
        ...
        /**
	 * Dołącza numer telefonu do łańcucha znaków
	 *
	 * @param string $phones łańcuch znaków z numerami telefonów
         * @param bool $isNotFirst czy nie jest to pierwszy numer telefonu w łańcuchu
			...
			if (!is_numeric($id))
			{
				...
				if (($info = $this->resource_info($id)))
				{
					...
					if ($info['name']) $id2email[$id] = $info['email'];
                                        if ($info['phones'] != '') //jeśli  kontakt ma numery telefonu, to dołącz spacje przed nimi
                                        {
                                            $phones = ' '.$info['phones'];
                                        }
				}
