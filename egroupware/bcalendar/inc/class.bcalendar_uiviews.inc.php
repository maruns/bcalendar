...
class bcalendar_uiviews extends bcalendar_ui {
  ...
  function __construct($set_states = null) {
    ...
    if ($GLOBALS['egw_info']['user']['preferences']['common']['enable_dragdrop']) {
      ...
      if (!$this->dragdrop->validateBrowser())
        $this->dragdrop = false;
    }
    $this->dragdrop = false;
  }
  ...
  function &planner($home = false) {
    if (!$this->planner_days) { // planner monthview
      ...
      $this->last['hour'] = 23;
      $this->last['minute'] = $this->last['sec'] = 59;
      unset($this->last['raw']);
  ...
  function &year($home = false) {
    ...
    for ($month = 1; $month <= 12; $month++) {
      ...
      $month_end = $this->datetime->get_weekday_start(
              $this->year, $month, $this->datetime->days_in_month($month, $this->year));
  ...
  function _week_align_month(&$first, &$last, $day = 1) {
    ...
    if ($day == 1) {
      $last = $this->datetime->get_weekday_start($this->year, $this->month, $this->datetime->days_in_month($this->month, $this->year));
    } else {
  ...
  function _month_align_year(&$first, &$last) {
    ...
    $last = $this->datetime->get_weekday_start($this->year, $this->month + 12, $this->datetime->days_in_month($this->month + 12, $this->year));
    // now we need to calculate the end of the last day of that week
    // as simple $last += WEEK_s - 1; does NOT work, if daylight saving changes in that week!!!
    $last = $this->bo->date2array($last);
    $last['day'] += 6;
  ...
  function week($days = 0, $home = false) {
    ...
    //    $content .= '<b>' . $label . "</b>\n";
    //    $content .= $this->timeGridWidget($this->tagWholeDayOnTop($this->bo->search($search_params)), count($users) * $this->cal_prefs['interval'], 400 / count($users), '', '', $uid);
    //  }
    //}
    $content .= $this->edit_series();
   ...
   <script type="text/javascript">
       var calendarExt = {
         scroll : function(div) {
           ...
           var hours = document.getElementById('calendar_hour_container');
          
           hours.style.top = 45-div.scrollTop+'px';
           
           var days = document.getElementById('calendar_days_header_container');
           var docs = document.getElementById('calendar_docs_header_container');
           days.style.left = -div.scrollLeft+'px';
           docs.style.left = -div.scrollLeft+'px';
         },
         mdown : function(e) {
            e = e || window.event;
            source = e.target || e.srcElement;
            if(source.id === 'calendar_data_container') {
              e.cancelBouble = true;
...
SCRIPT;
    ...
    //echo "NumberOfDays: "+$numberOfDays;
    $docWidth = 175;
    $user_allowed_list = $this->_get_planner_users();
    $userCount = count($user_allowed_list);
    $dayColWidth = $docWidth*$userCount+$userCount-2; //userCount - 2 korekta na obramowanie
...
//		$this->rowHeight		= round(100/$this->rowsToDisplay,1);
    ...
    foreach ($daysEvents as $day => $events) {
        ...
        $day_html .= $this->dayColHeaderWidget($day, $events, $n * $dayColWidth, $dayColWidth, $indent . "\t\t", $short_title, ++$on_off & 1, $col_owner);
        ++$n;
        
        foreach($user_allowed_list as $user => $name) {
          $docs_html .= '<div style="display:inline-block; width:'.$docWidth.'px;height: 15px;padding-left: 0px;border-left:1px solid gray;">'.$name.'</div>';
          //var_dump($user);
          //var_dump($name);
        }
      }
    $html .= $indent . "\t" . '<div class="calGridHeader" style="height: 45px; background-color:white; padding-left: 45px; overflow: hidden;">' .
            '<div id="calendar_title_container" style="height: 15px">' . $title . "</div>".
            '<div id="calendar_days_header_container" style="position:relative; height: 15px;">' . $day_html . "</div>".
            '<div id="calendar_docs_header_container" style="position:relative; height: 15px">' . $docs_html . "</div>".
            "</div>\n";
    ...
    if ($this->use_time_grid) {
      ...
      // kolumna z godzinami
      $html .= '<div id="calendar_hour_container" style="position: absolute; top: 45px; left: 0px; right: 5px; bottom: 20px; width: 50px;">';
      for ($t = $this->scroll_to_wdstart ? 0 : $this->wd_start, $i = 1 + $this->extraRows; $t <= $this->wd_end || $this->scroll_to_wdstart && $t < 24 * 60; $t += $this->granularity_m, ++$i) {
        $set_id = '';
        if ($t == $this->wd_start) {
        ...
        $html .= $indent . "\t" . '<div' . $set_id . ' class="' . ($off ? 'Off row_off' : ' row_on') .
                '" style="height: ' . $this->rowHeight . 'px; width: 50px;" >' . "\n";
    ...
    45 => array(0, 15, 30, 45),
      ...
      $html .= '</div>';
      $off = false; // Off-row means a fferent bgcolor
      $add_links = count($daysEvents) == 1;

      // dane
      $data_width = $dayColWidth * $numberOfDays;
      $html .= $indent . "\t<div id='calendar_data_container' style='position: absolute; top: 45px; left:45px; right: 0px; overflow: scroll; bottom: 0px' onscroll='calendarExt.scroll(this)' onmousedown='calendarExt.mdown(event)'>";
      $html .= "<div style='position:absolute; top: 0px; left: 0px; width:" . $data_width . "px;'>\n";
      ...
      for ($t = $this->scroll_to_wdstart ? 0 : $this->wd_start, $i = 1 + $this->extraRows; $t <= $this->wd_end || $this->scroll_to_wdstart && $t < 24 * 60; $t += $this->granularity_m, ++$i) {
        ...
        $html .= $indent . "\t" . '<div' . $set_id . ' class="' . ($off ? 'Off row_off' : ' row_on') .
                '" style="height: ' . $this->rowHeight . 'px; top:' . $i * $this->rowHeight . 'px;">' . "\n";
        // show time for full hours, allways for 45min interval and at least on every 3 row
        $html .= $indent . "\t</div>\n"; // calTimeRow
        $off = !$off;
      }
...
//
      $html .= $indent . "\t" . '<div id="calDayCols" class="calDayCols' .
              ($this->use_time_grid ? ($this->bo->common_prefs['timeformat'] == 12 ? '12h' : '') : 'NoTime') . '" style="left:0px;">' . "\n";
//
      ...
      $n = 0;
      $u = 0; //numer kolumny w danym dniu
      foreach ($daysEvents as $day => $events) {
        foreach($events as $event) {
          $userEvents[$event['owner']][] = $event;
        }
        foreach($user_allowed_list as $user => $name) {
          $this->wholeDayPosCounter = 1;
          $short_title = count($daysEvents) > 1;
  ...
  function dayColHeaderWidget($day_ymd, $events, $left, $width, $indent, $short_title = True, $on_off = False, $owner = 0) {
    $html = '';
    ...
    if ($short_title === true) {
      if ($this->allowEdit) {
        $title = html::a_href($title, $day_view, '', !isset($this->holidays[$day_ymd]) ? ' title="' . lang('Dayview') . '"' : '');
      }
    ...
    if (is_bool($short_title) || ($short_title != "")) {
      $html .= '<span style="height: 15px; width:'.$width.'px; position:relative; display:inline-block;" class="calDayColHeader ' . $class . '"' .
              ($holidays ? ' title="' . html::htmlspecialchars($holidays) . '"' : '') . '>' . $title . "</span>";
    }
    return $html;
  }
   ...
   * @param bool $isFirst czy kolumna jest pierwsza, domyślnie nie
   */
  function dayColWidget($day_ymd, $events, $left, $width, $indent, $short_title = True, $on_off = False, $owner = 0, $isFirst = false) {
    ...
    $html_outer = $indent . '<div id="calColumn' . $this->calColumnCounter++ . '" class="calDayCol" style="'.$fc.'left: ' . $left .
            'px; width: ' . $width . 'px; height: '; //ze zmienionym obramowaniem dla pierwszej kolumny
    ...
    foreach ($eventCols as $n => $eventCol) {
      ...
      if ($left + $width > 100.0)
        $width = 100.0 - $left;
      $html .= $this->eventColWidget($eventCol, $left, $width, $indent . "\t", $owner ? $owner : $this->user, 20 + 10 * $n);
    }
    
    $html_outer .= ( $input_cnt*$this->rowHeight ).'px">' . "\n";
    $html .= $indent . "</div>\n"; // calDayCol

    return $html_outer.$html;
  }
  ...
  function eventColWidget($events, $left, $width, $indent, $owner, $z_index = null) {
    ...
    $html = $indent . '<div class="calEventCol" style="left: ' . $left . '%; width:' . $width . '%;' .
            ...
            (!$this->use_time_grid ? ' top: ' . $this->rowHeight . 'px;' : '') . '">' . "\n";
    foreach ($events as $event) {
  ...
  function eventWidget($event, $width, $indent, $owner, $return_array = false, $block = 'event_widget', $z_index = null) {
    ...
    $tpl->set_var(array(
        ...
        'icons' => implode('', $icons),
        'timespan' => $timespan,
        'title' => nl2br($patients).($title = !$is_private ? html::htmlspecialchars($event['title']) : lang('private')),
...
//			'tooltip' => html::tooltip(False,False,array('BorderWidth'=>0,'Padding'=>0)),
    ...
    if (html::$user_agent == 'msie') { // add a transparent image to make the event "opaque" to mouse events
      $ie_fix = $indent . "\t" . html::image('calendar', 'transparent.gif', '', html::tooltip($tooltip, False, $ttip_options) .
                      ' style="top:0px; left:0px; position:absolute; height:100%; width:100%; z-index:1"') . "\n";
    }
    if ($this->use_time_grid) {
      if ($event['whole_day_on_top']) {
        $style = 'top: ' . ($this->rowHeight * $this->wholeDayPosCounter) . 'px; height: ' . $this->rowHeight . 'px;';
        $this->wholeDayPosCounter++;
      } else {
        $style = 'top: ' . $this->time2pos($event['start_m']) . 'px; height: ' . $height . 'px;';
      }
    } else {
    ...
    if ($by_cat === false) { // planner by user
      ...
      switch ($this->filter) {
        case 'unknown':
          ...
          break;
        case 'accepted':
          $status_to_show = array('A');
          break;
        case 'tentative':
          $status_to_show = array('T');
          break;
        case 'rejected':
          $status_to_show = array('R');
          break;
        case 'delegated':
          $status_to_show = array('D');
          break;
        case 'all':
          $status_to_show = array('U', 'A', 'T', 'D', 'G', 'R');
          break;
        default:
          $status_to_show = array('U', 'A', 'T', 'D', 'G');
          break;
      }
    }
    ...
    for ($t = $start, $left = 0, $i = 0; $i < $days; $t += $days_in_month * DAY_s, $left += $days_in_month * $day_width, $i += $days_in_month) {
      ...
      if ($days_in_month > 10) {
        ...
        if ($this->bo->date2ts($prev) < $start - 20 * DAY_s) {
          ...
          if ($this->day >= 15)
            $prev = $t_arr;  // we stay in the same month
        ...
        if ($days_in_next_month <= 10 || date('m', $end) == date('m', $t)) {
          if ($this->day >= 15)
            $next = $t_arr;  // we stay in the same month
          $next['day'] = $this->day;
  ...
  function plannerRowWidget($events, $start, $end, $header, $class, $indent = '') {
    ...
    foreach ($events as $n => $event) {
      for ($row = 0; (int) $row_end[$row] > $event['start']; ++$row)
