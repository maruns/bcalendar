	...
	function get_rows(&$query,&$rows,&$readonlys,$id_only=false)
	{
		...
		elseif(!$query['search'] && $old_state['advanced_search'])	// eg. paging in an advanced search
		{
			$query['advanced_search'] = $old_state['advanced_search'];
                ...
                if (!$GLOBALS['egw_info']['user']['apps']['admin'] && !$query['search'] && 
                    (!$query['advanced_search'] || count($query['advanced_search']) < 3) && 
                    !$query['searchletter']) //nie pokazuj wszystkich kontaktów, jeśli nie jest zalogowany administrator
                {
                    return 0;
                }
