 ...
 * @version $Id: class.addressbook_bo.inc.php 39311 2012-05-23 08:53:53Z jlehrke $
 */
...
class addressbook_bo extends addressbook_so
{
	...
	*/
	 ...
	 */
        ...
        /**
	 * Zwraca informacje o kontaktach z numerami telefonów
	 *
	 * @param int/array $ids single contact-id or array of id's
	 * @return array
	 */
        function GetCalendarInfoWithPhones($ids)
        {
            if (!$ids) return null;
            ...
            foreach(!is_array($ids) ? array($ids) : $ids as $id)
            {
                    ...
                    $data[] = array(
                            ...
                            'phones' => $phones //numer tlefonu w zwracanej tablicy
                    );
            }
		...
		foreach(!is_array($ids) ? array($ids) : $ids as $id)
		{
			if (!($contact = $this->read($id))) continue;/*
                        ...
                        else
                        {
                            ...
                            if (isset($contact['tel_cell_private']))
                            {
                                $phone = $contact['tel_cell_private'].' (tel. prywatny)';
                                $IsNotFirst = true;
                            }
                            $this->AppendPhoneNumber($phones, $IsNotFirst, $contact['tel_cell'], ' (tel. kom.)');
                            $this->AppendPhoneNumber($phones, $IsNotFirst, $contact['tel_work'], ' (tel. w firmie)');
